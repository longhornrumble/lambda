...
/**
 * aws-client-config.js — shared AWS SDK v3 client config for Calendar_Event_Consumer.
 *
 ...
 * the Lambda timeout) so a hang fails fast into the SQS redrive path. Mirrors the C8
 * `Booking_Commit_Handler/aws-client-config.js` convention (each Lambda owns its own copy
 * — never a shared cross-Lambda module).
 */
