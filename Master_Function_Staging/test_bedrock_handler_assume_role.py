"""
Tests for the cross-account assume-role KB client factory in bedrock_handler.py
and bedrock_handler_optimized.py.
...
class TestBedrockHandlerAssumeRole:
    """Tests for bedrock_handler._get_bedrock_agent_client()"""

    def setup_method(self):
        _reset_cache(bh)

    # ------------------------------------------------------------------
    # Case 1: env unset → STS not called, default-creds client returned
    # ------------------------------------------------------------------

    def test_env_unset_no_sts_call(self, monkeypatch):
        monkeypatch.delenv("KB_RETRIEVER_ROLE_ARN", raising=False)

        fake_client = MagicMock(name="bedrock-agent-default")
        with patch("bedrock_handler.boto3") as mock_boto3:
            mock_boto3.client.return_value = fake_client
            result = bh._get_bedrock_agent_client()

        # boto3.client called once for "bedrock-agent-runtime", NOT for "sts"
        assert mock_boto3.client.call_count == 1
        mock_boto3.client.assert_called_once_with("bedrock-agent-runtime")
        assert result is fake_client

    # ------------------------------------------------------------------
    # Case 2: env set → STS assume_role called with correct args,
    #         returned client uses temp creds
    # ------------------------------------------------------------------

    def test_env_set_assumes_role(self, monkeypatch):
        monkeypatch.setenv("KB_RETRIEVER_ROLE_ARN", ROLE_ARN)

        fake_bedrock_client = MagicMock(name="bedrock-agent-assumed")
        mock_sts = MagicMock()
        mock_sts.assume_role.return_value = FAKE_CREDS

        with patch("bedrock_handler.boto3") as mock_boto3:
            # First call → "sts", second call → "bedrock-agent-runtime"
            mock_boto3.client.side_effect = [mock_sts, fake_bedrock_client]
            result = bh._get_bedrock_agent_client()

        # STS called correctly
        ...
        # bedrock-agent-runtime client built with temp creds
    ...
    # ------------------------------------------------------------------
    # Case 3: env set, second call within TTL → STS NOT called again
    # ------------------------------------------------------------------

    def test_env_set_client_cached(self, monkeypatch):
        monkeypatch.setenv("KB_RETRIEVER_ROLE_ARN", ROLE_ARN)

        fake_bedrock_client = MagicMock(name="bedrock-agent-cached")
        mock_sts = MagicMock()
        mock_sts.assume_role.return_value = FAKE_CREDS

        with patch("bedrock_handler.boto3") as mock_boto3:
            mock_boto3.client.side_effect = [mock_sts, fake_bedrock_client]
            first = bh._get_bedrock_agent_client()

        # Warm the cache manually (set expires_at far in the future)
        bh._kb_client_cache["client"] = fake_bedrock_client
        bh._kb_client_cache["expires_at"] = time.time() + 3000

        with patch("bedrock_handler.boto3") as mock_boto3_second:
            second = bh._get_bedrock_agent_client()

        # boto3 not called on the second invocation
        mock_boto3_second.client.assert_not_called()
        assert second is fake_bedrock_client

    # ------------------------------------------------------------------
    # Case 4: env set, STS raises → log line emitted, exception propagates
    # ------------------------------------------------------------------
...
# ===========================================================================
# bedrock_handler_optimized.py — identical cases, same factory pattern
# ===========================================================================
