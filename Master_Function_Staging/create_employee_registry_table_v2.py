#!/usr/bin/env python3
"""
DynamoDB Table Creation Script for PICASSO Employee Registry v2
Employee Registry v2 — UUID-based employeeId sort key, supports non-Clerk users.

Usage:
    python create_employee_registry_table_v2.py create
    python create_employee_registry_table_v2.py verify
    python create_employee_registry_table_v2.py delete
...
TABLE_NAME = f"picasso-employee-registry-v2-{ENVIRONMENT}"
...
def main():
    ...
    print("PICASSO Employee Registry v2 - Table Management")
    print("=" * 50)
    print(f"Environment: {ENVIRONMENT}")
    print(f"Region: {AWS_REGION}")
    ...
    if command == 'create':
        ...
        print(f"  Sort Key: employeeId (String, UUID)")
        ...
        print("  - ClerkUserIdIndex (PK: clerkUserId) — Clerk webhook lookups (sparse: null clerkUserId excluded automatically)")
        print("\nNon-Key Attributes (stored but not indexed):")
        print("  - name, role (admin/member), type (clerk_user/local_only)")
        print("  - status (active/invited/inactive)")
        print("  - phone (nullable, E.164)")
        print("  - notificationPrefs (nullable, Map: {email, sms, sms_quiet_hours: {enabled, start, end, timezone, fallback_to_email}})")
