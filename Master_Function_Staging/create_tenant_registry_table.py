#!/usr/bin/env python3
"""
DynamoDB Table Creation Script for PICASSO Tenant Registry
Canonical source of truth for tenant identity — replaces Bubble's implicit registry role.

Usage:
    python create_tenant_registry_table.py create
    python create_tenant_registry_table.py verify
    python create_tenant_registry_table.py delete
...
def create_tenant_registry_table():
    """Create the tenant registry DynamoDB table with proper configuration"""
    ...
    table_config = {
        ...
        'AttributeDefinitions': [
            ...
            {
                'AttributeName': 'tenantHash',
                'AttributeType': 'S'
            },
            {
                'AttributeName': 'clerkOrgId',
                'AttributeType': 'S'
            },
            {
    ...
    try:
        print(f"Creating tenant registry table: {TABLE_NAME}")
        ...
        # No TTL — tenant records are persistent

        print(f"Tenant registry table {TABLE_NAME} created and configured successfully!")
        ...
        print(f"  GSIs: TenantHashIndex, ClerkOrgIdIndex, StripeCustomerIdIndex")
...
def verify_tenant_registry_table():
    """Verify the tenant registry table exists and is properly configured"""
...
def delete_tenant_registry_table():
    """Delete the tenant registry table (for cleanup/testing)"""

    dynamodb = boto3.client('dynamodb', region_name=AWS_REGION)

    print(f"WARNING: About to delete table {TABLE_NAME}")
    print("This will permanently delete all tenant registry data!")
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage:")
        print(f"  {sys.argv[0]} create    - Create tenant registry table")
    ...
    print("PICASSO Tenant Registry - Table Management")
    print("=" * 50)
    print(f"Environment: {ENVIRONMENT}")
    print(f"Region: {AWS_REGION}")
    ...
    if command == 'create':
        success = create_tenant_registry_table()
        sys.exit(0 if success else 1)

    elif command == 'verify':
        success = verify_tenant_registry_table()
        sys.exit(0 if success else 1)

    elif command == 'delete':
        success = delete_tenant_registry_table()
        sys.exit(0 if success else 1)
        ...
        print("  - TenantHashIndex (PK: tenantHash) — widget hash resolution")
        print("  - ClerkOrgIdIndex (PK: clerkOrgId) — Clerk webhook lookups")
        print("  - StripeCustomerIdIndex (PK: stripeCustomerId) — Stripe webhook lookups")
        print("\nNon-Key Attributes (stored but not indexed):")
        print("  - companyName, networkId, networkName")
        print("  - s3ConfigPath, subscriptionTier, status")
        print("  - onboardedAt, updatedAt")
        print("\nEnvironment Variable:")
        print(f"  TENANT_REGISTRY_TABLE={TABLE_NAME}")
