...
| Errors (to model) | `{ error: 'no_availability' \| 'lookup_failed', note: string }`. Model instructed to apologise honestly and offer the email fallback; never to fabricate times. (`lookup_failed` also covers: propose seam unwired, unresolvable appointment type, invalid model-supplied `date`, illegal state for a re-propose — fail-closed within the closed vocabulary.) |
