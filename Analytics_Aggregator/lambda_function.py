"""
Analytics Aggregator Lambda — Attribution Monthly Rollups
=========================================================

Computes attribution aggregate rows per FROZEN_CONTRACTS C5 and writes them
to picasso-dashboard-aggregates.  Runs hourly via EventBridge (same schedule
as the existing aggregations).  Also polls Dub.co analytics per C4.

New aggregate SK patterns (C5):
  METRIC#attribution_summary#{YYYY-MM}
  METRIC#attribution_channel#{YYYY-MM}#{channel}
  METRIC#attribution_entrypoint#{YYYY-MM}#{entry_point_id}

TTL: now + 420 days (attribute name: "ttl" — confirmed from existing store_metric).

Environment variables required (new — see report-back):
  ENTRY_POINTS_TABLE   DynamoDB table name for picasso-entry-points-{env}
  DUB_SECRET_NAME      Secrets Manager secret name for the Dub API key
  SESSION_EVENTS_TABLE (default: picasso-session-events)
  AGGREGATES_TABLE     (default: picasso-dashboard-aggregates)
  CONFIG_BUCKET        (default: myrecruiter-picasso)
...
# C7 — staff-hours idle cap
IDLE_CAP_SECONDS = 5 * 60       # 5 minutes — FROZEN_CONTRACTS C7 (C14 amendment)
...
def lambda_handler(event: Dict, context: Any) -> Dict:
    ...
    single-tenant re-run.
    C8.10: log IDs and counts only — never full event payloads at INFO.
    """
    logger.info(
        'Analytics Aggregator (attribution) started: tenants=%s',
    ...
    logger.info('Processing %d tenants', len(tenants))

    results = []
    for tenant_id in tenants:
        try:
            metrics_updated = _aggregate_attribution(tenant_id)
...
# ---------------------------------------------------------------------------
def _aggregate_attribution(tenant_id: str) -> int:
    ...
    for month_start in months_to_compute:
        metrics_written += _compute_month(tenant_id, month_start, tz, config)
...
def _compute_month(
    ...
    # Fetch all session events for this tenant × month window
    events = _fetch_session_events(tenant_id, start_iso, end_iso)
...
# ---------------------------------------------------------------------------
def _fetch_session_events(tenant_id: str, start_iso: str, end_iso: str) -> List[Dict]:
    """
    Query picasso-session-events via tenant-date-index GSI.
    Keys: tenant_id (PK), timestamp (SK).
    ...
    try:
        ...
        logger.error('DynamoDB query failed for tenant=%s: %s', tenant_id[:8], exc)
...
def _get_entry_point(tenant_id: str, entry_point_id: str) -> Optional[Dict]:
    """
    Schema-tolerant read of picasso-entry-points-{env} (C3).
...
def _store_metric(tenant_id: str, metric_key: str, data: Dict, ttl: int) -> None:
    """
    Write to picasso-dashboard-aggregates.
    PK = TENANT#{tenant_id}, SK = METRIC#{metric_key}.
    TTL attribute = 'ttl' (confirmed from existing Aggregator store_metric).
    """
    table = _dynamodb_resource.Table(AGGREGATES_TABLE)
    item = {
...
# ---------------------------------------------------------------------------
def _get_active_tenants() -> List[str]:
    """Enumerate active tenant IDs from S3 config bucket (existing pattern)."""
    try:
        resp = _s3_client.get_object(Bucket=CONFIG_BUCKET, Key='mappings.json')
        mappings = json.loads(resp['Body'].read().decode('utf-8'))
